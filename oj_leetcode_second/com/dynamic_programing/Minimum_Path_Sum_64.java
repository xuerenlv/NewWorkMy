...
class Solution_Minimum_Path_Sum_64 {
	...
	void trival(int[][] grid, int start_r, int start_c, int m, int n, Integer this_sum, int[] re) {
		...
		if (start_r < m) {
			trival(grid, start_r + 1, start_c, m, n, this_sum + grid[start_r+1][start_c], re);
		}
		if (start_c < n) {
			trival(grid, start_r, start_c + 1, m, n, this_sum + grid[start_r][start_c+1], re);
		}
